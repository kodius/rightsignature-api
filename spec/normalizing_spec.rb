...
describe RightSignature2013::Helpers::TagsHelper do 
  describe "mixed_array_to_string_array" do
    it "should convert array of strings and hashes to string" do
      RightSignature2013::Helpers::TagsHelper.mixed_array_to_string_array(["a", {'good' => "yea"}, '123']).should == "a,good:yea,123"
    end
    
    it "should raise error if array contains something other than hash or string" do
      lambda {RightSignature2013::Helpers::TagsHelper.mixed_array_to_string_array(["a", ['bad'], {'good' => "yea"}])}.should raise_error
    end
  end
  ...
  describe "array_to_xml_hash" do
    it "should convert array of string and hash of {name => value} into array of :tag => {:name => name, :value => value}" do
      RightSignature2013::Helpers::TagsHelper.array_to_xml_hash(['abc', {"taggy" => "tvalue"}]).should == [{:tag => {:name => 'abc'}}, {:tag => {:name => 'taggy', :value => 'tvalue'}}]
    end
    
    it "should convert a symbol to a string for names and values" do
      RightSignature2013::Helpers::TagsHelper.array_to_xml_hash([:abc, {:taggy => :tvalue}]).should == [
        {:tag => {:name => 'abc'}}, 
...
describe RightSignature2013::Helpers::MergeFieldsHelper do
  describe "array_to_xml_hash" do
    it "should convert array of {\"Merge field name\" => \"Merge Field Value\"} to array of {:merge_field => {:value => \"Merge Field Value\", \"@merge_field_name\" => \"Merge Field Name\"}} " do
      results = RightSignature2013::Helpers::MergeFieldsHelper.array_to_xml_hash([
        {"City" => "Santa Barbara"},
        {"Section" => "House"}
      ])
      results.size.should == 2
