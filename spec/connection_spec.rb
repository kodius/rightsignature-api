...
describe RightSignature2013::Connection do
  before do
  ...
  describe "GET" do
    describe "connection method" do
      it "should default to RightSignature2013::57 if no api_token was specified" do
        @rs = RightSignature2013::Connection.new({:consumer_key => "Consumer123", :consumer_secret => "Secret098", :access_token => "AccessToken098", :access_secret => "AccessSecret123"})
      ...
      it "should use Token Connection if api_token was specified" do
        @rs = RightSignature2013::Connection.new({:api_token => "APITOKEN", :consumer_key => "Consumer123", :consumer_secret => "Secret098", :access_token => "AccessToken098", :access_secret => "AccessSecret123"})
      ...
      it "should use Token Connection if only api_token was specified" do
        @rs = RightSignature2013::Connection.new({:api_token => "APITOKEN"})
        @rs.token_connection.should_receive(:request).and_return(@httparty_response)
        @rs.get("/path")
      end
    end
    ...
    describe "using TokenConnection" do
      it "should append params and header into query option and header option" do
        @rs = RightSignature2013::Connection.new({:api_token => 'token'})
